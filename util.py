...
def compute_cost(predicted_ys: torch.Tensor, actual_ys: torch.Tensor) -> torch.Tensor:
    """
...
def paint_reliability_diagram(out, title="Reliability Diagram", xlabel="Confidence", ylabel="Accuracy"):
...
def calculate_calibration_curve(predicted_probabilities: np.ndarray, labels: np.ndarray, num_bins: int) -> plt.figure:
    """
